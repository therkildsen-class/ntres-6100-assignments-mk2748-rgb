...
ggplot(economist_data, aes(x = HDI, y = CPI)) + geom_point(color =
“red”)
...
ggplot(economist_data, aes(x = CPI, y = HDI)) + geom_point(aes(color =
0)) +geom_smooth(method = “lm”, se = FALSE) + facet_wrap(~Region)
...
ggplot(economist_data, aes(x = HDI, fill = Region)) + geom_density(alpha
= 0.5)
...
geom_histogram(color = 0) + facet_wrap(~Region)
...
ggplot(economist_data, aes(x = Region, y = HDI, fill = Region)) +
geom_boxplot(aes(color = Region), alpha = 0.5, outlier.shape = NA) +
geom_jitter(aes(color = Region), width = 0.2, size = 2, alpha = 0.7)
...
sum_dose = sum(Dose)) %\>% arrange(sum_dose) %\>% slice_head(n = 6) %\>%
kable()
